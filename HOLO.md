...
| `as_npc_detail` | `1.5` | 1–6 | Splat spacing **on monsters only**, in Quake units. Lower = denser = a bigger death burst and more skin detail, at a quadratic cost in samples. With `as_npc_solid` on, a monster's samples are spent entirely on its death burst, so a finer spacing costs nothing until something dies. Weapons and pickups keep the original 2.5 because they draw as point clouds every frame. Baked at model load, so it needs a map reload. Past the 250k-per-model cap a model falls back to solid triangles and prints a warning. |
...
| `as_gib_shards` | `1` | boolean | Two-stage death. A killed enemy bursts into a sparse set of large shards, then at 20% of `r_holo_phys_death_life` they shatter into the full cloud of tiny fragments, which scatter and bounce off walls, ceilings and the floor. One particle population in two phases, not a secondary spawn. 0 bursts straight into fragments as before. |
