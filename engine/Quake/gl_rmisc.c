...
void R_CreateDescriptorSetLayouts ()
{
...

	{
		...
		ubo_layout_bindings.stageFlags = VK_SHADER_STAGE_ALL_GRAPHICS | VK_SHADER_STAGE_COMPUTE_BIT;
...
#include "../Shaders/Compiled/aftershock.vert.c"
#include "../Shaders/Compiled/aftershock.frag.c"
#include "../Shaders/Compiled/aftershock_structure.vert.c"
DECLARE_SHADER_MODULE (aftershock_structure_vert);
DECLARE_SHADER_MODULE (aftershock_vert);
#include "../Shaders/Compiled/holo_splat.c"
#include "../Shaders/Compiled/holo_structure.c"
DECLARE_SHADER_MODULE (holo_splat);
DECLARE_SHADER_MODULE (holo_structure);
#include "../Shaders/Compiled/holo_fragment.c"
DECLARE_SHADER_MODULE (holo_fragment);
#include "../Shaders/Compiled/holo_gore_vert.c"
#include "../Shaders/Compiled/holo_gore_frag.c"
DECLARE_SHADER_MODULE (holo_gore_vert);
DECLARE_SHADER_MODULE (holo_gore_frag);
DECLARE_SHADER_MODULE (aftershock_frag);
#include "../Shaders/Compiled/aftershock_fx.vert.c"
DECLARE_SHADER_MODULE (aftershock_fx_vert);
#include "../Shaders/Compiled/aftershock_alias.vert.c"
DECLARE_SHADER_MODULE (aftershock_alias_vert);
#include "../Shaders/Compiled/aftershock_alias.frag.c"
DECLARE_SHADER_MODULE (aftershock_alias_frag);
...
static void R_CreateParticlesPipelines ()
{
	...
	/* Procedural GPU particles share Quake's transparency and fog contracts. */
	VkVertexInputBindingDescription fx_binding={0,0,VK_VERTEX_INPUT_RATE_INSTANCE};
	VkVertexInputAttributeDescription fx_attrs[5]={{0,0,VK_FORMAT_R32G32B32A32_SFLOAT,0},{1,0,VK_FORMAT_R32G32B32A32_SFLOAT,16},{2,0,VK_FORMAT_R32G32B32A32_SFLOAT,32},{3,0,VK_FORMAT_R32G32B32A32_SFLOAT,48},{4,0,VK_FORMAT_R32G32B32A32_SFLOAT,64}};
	base.vertex_input_state.vertexBindingDescriptionCount=1;base.vertex_input_state.pVertexBindingDescriptions=&fx_binding;
	base.vertex_input_state.vertexAttributeDescriptionCount=5;base.vertex_input_state.pVertexAttributeDescriptions=fx_attrs;
	base.shader_stages[0].module=aftershock_fx_vert_module;base.rasterization_state.cullMode=VK_CULL_MODE_NONE;
	for(int variant=0;variant<MAIN_RENDER_PASS_VARIANT_COUNT;++variant){R_CopyPipelineCreateInfos(&infos,&base);R_SetPipelineRenderPassVariant(&infos,SUBPASS_MAIN,variant);R_CreateGraphicsPipeline(&vulkan_globals.aftershock_fx[variant],&infos,vulkan_globals.basic_pipeline_layout,"aftershock_fx");}
	R_CopyPipelineCreateInfos(&infos,&base);R_SetPipelineRenderPassVariant(&infos,SUBPASS_WBOIT,MAIN_RENDER_PASS_OIT);infos.color_blend_state.attachmentCount=WBOIT_COLOR_ATTACHMENT_COUNT;infos.shader_stages[1].module=basic_oit_frag_module;R_SetWBOITBlend(infos.blend_attachment_states);R_CreateGraphicsPipeline(&vulkan_globals.aftershock_fx_oit,&infos,vulkan_globals.basic_pipeline_layout,"aftershock_fx_oit");
	R_CopyPipelineCreateInfos(&infos,&base);R_SetPipelineRenderPassVariant(&infos,SUBPASS_MBOIT_MOMENTS,MAIN_RENDER_PASS_MBOIT);infos.color_blend_state.attachmentCount=MBOIT_MOMENT_COLOR_ATTACHMENT_COUNT;infos.shader_stages[1].module=basic_mboit_moment_frag_module;R_SetMBOITMomentBlend(infos.blend_attachment_states);R_CreateGraphicsPipeline(&vulkan_globals.aftershock_fx_moment,&infos,vulkan_globals.basic_pipeline_layout,"aftershock_fx_moment");
	R_CopyPipelineCreateInfos(&infos,&base);R_SetPipelineRenderPassVariant(&infos,SUBPASS_MBOIT_COMPOSITE,MAIN_RENDER_PASS_MBOIT);infos.color_blend_state.attachmentCount=MBOIT_COMPOSITE_COLOR_ATTACHMENT_COUNT;infos.shader_stages[1].module=(vulkan_globals.sample_count==VK_SAMPLE_COUNT_1_BIT)?basic_mboit_composite_frag_module:basic_mboit_composite_msaa_frag_module;R_SetMBOITCompositeBlend(infos.blend_attachment_states);R_CreateGraphicsPipeline(&vulkan_globals.aftershock_fx_composite,&infos,vulkan_globals.basic_pipeline_layout,"aftershock_fx_composite");
}
...
static void R_CreateWorldPipelines ()
{
	...
	HP_CreateLayout();
	/* Surface samples are instance records; four indexed corners expand on GPU. */
	for (int variant = 0; variant < MAIN_RENDER_PASS_VARIANT_COUNT; ++variant)
	{
		pipeline_create_infos_t sample;
		R_CopyPipelineCreateInfos (&sample, &base);
		R_SetPipelineRenderPassVariant (&sample, SUBPASS_MAIN, variant);
		VkVertexInputBindingDescription bindings[2] = {{0,sizeof(as_sample_t),VK_VERTEX_INPUT_RATE_INSTANCE},{1,0,VK_VERTEX_INPUT_RATE_INSTANCE}};
		VkVertexInputAttributeDescription attrs[7] = {{0,0,VK_FORMAT_R32_UINT,0},{1,1,VK_FORMAT_R32G32B32A32_SFLOAT,offsetof(as_surface_gpu_t,origin)},{2,1,VK_FORMAT_R32G32B32A32_SFLOAT,offsetof(as_surface_gpu_t,saxis)},{3,1,VK_FORMAT_R32G32B32A32_SFLOAT,offsetof(as_surface_gpu_t,taxis)},{4,1,VK_FORMAT_R32G32B32A32_SFLOAT,offsetof(as_surface_gpu_t,tex)},{5,1,VK_FORMAT_R32G32B32A32_SFLOAT,offsetof(as_surface_gpu_t,light)},{6,1,VK_FORMAT_R32G32B32A32_SFLOAT,offsetof(as_surface_gpu_t,params)}};
		sample.vertex_input_state.vertexBindingDescriptionCount = 2;
		sample.vertex_input_state.pVertexBindingDescriptions = bindings;
		sample.vertex_input_state.vertexAttributeDescriptionCount = 7;
		sample.vertex_input_state.pVertexAttributeDescriptions = attrs;
		sample.shader_stages[0].module = aftershock_vert_module;
		sample.shader_stages[1].module = aftershock_frag_module;
		sample.rasterization_state.cullMode = VK_CULL_MODE_NONE;
		sample.rasterization_state.depthBiasEnable = VK_FALSE;
		for(int full=0;full<2;++full) {
			specialization_data[0]=full;
			R_CreateGraphicsPipeline(&vulkan_globals.aftershock_splat[variant][full],&sample,vulkan_globals.world_pipeline_layout,"aftershock_splats");
            sample.shader_stages[0].module=holo_splat_module;
            sample.shader_stages[1].module=holo_fragment_module;
            R_CreateGraphicsPipeline(&vulkan_globals.holo_splat[variant][full],&sample,vulkan_globals.holo_world_layout,"holo_splat");
            sample.shader_stages[0].module=aftershock_vert_module;
            sample.shader_stages[1].module=aftershock_frag_module;
		}

        VkVertexInputAttributeDescription structure_attrs[10];
        memcpy(structure_attrs, attrs, sizeof(attrs));
        structure_attrs[7] = (VkVertexInputAttributeDescription){7,1,VK_FORMAT_R32G32B32A32_SFLOAT,96};
        structure_attrs[8]=(VkVertexInputAttributeDescription){8,1,VK_FORMAT_R32G32B32A32_SFLOAT,112};
        structure_attrs[9]=(VkVertexInputAttributeDescription){9,1,VK_FORMAT_R32G32B32A32_SFLOAT,128};
        sample.vertex_input_state.vertexAttributeDescriptionCount = 10;
        sample.vertex_input_state.pVertexAttributeDescriptions = structure_attrs;
        sample.shader_stages[0].module = aftershock_structure_vert_module;
        for(int full=0;full<2;++full) {
            specialization_data[0]=full;
            R_CreateGraphicsPipeline(&vulkan_globals.aftershock_structure[variant][full],&sample,vulkan_globals.world_pipeline_layout,"aftershock_structure");
            sample.shader_stages[0].module=holo_structure_module;
            sample.shader_stages[1].module=holo_fragment_module;
            R_CreateGraphicsPipeline(&vulkan_globals.holo_structure[variant][full],&sample,vulkan_globals.holo_world_layout,"holo_structure");
            sample.shader_stages[0].module=aftershock_structure_vert_module;
            sample.shader_stages[1].module=aftershock_frag_module;
        }
        sample.shader_stages[0].module=holo_gore_vert_module;
        sample.shader_stages[1].module=holo_gore_frag_module;
        sample.shader_stages[1].pSpecializationInfo=NULL;
        sample.vertex_input_state.vertexBindingDescriptionCount=0;
        sample.vertex_input_state.vertexAttributeDescriptionCount=0;
        sample.depth_stencil_state.depthWriteEnable=VK_FALSE;
        sample.blend_attachment_states[0].blendEnable=VK_TRUE;
        sample.blend_attachment_states[0].srcColorBlendFactor=VK_BLEND_FACTOR_SRC_ALPHA;
        sample.blend_attachment_states[0].dstColorBlendFactor=VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
        sample.blend_attachment_states[0].colorBlendOp=VK_BLEND_OP_ADD;
        R_CreateGraphicsPipeline(&vulkan_globals.holo_gore[variant],&sample,vulkan_globals.holo_world_layout,"holo_gore");
		R_InitDefaultStates(&sample);
		R_SetPipelineRenderPassVariant(&sample,SUBPASS_MAIN,variant);
		sample.depth_stencil_state.depthTestEnable=VK_TRUE;
		sample.depth_stencil_state.depthWriteEnable=VK_TRUE;
		sample.rasterization_state.cullMode=VK_CULL_MODE_NONE;
		R_CreateGraphicsPipeline(&vulkan_globals.aftershock_solid[variant],&sample,vulkan_globals.basic_pipeline_layout,"aftershock_chunks");
	}
...
static void R_CreateAliasPipelines ()
{
	...
	for(int variant=0;variant<MAIN_RENDER_PASS_VARIANT_COUNT;++variant)for(int alpha=0;alpha<2;++alpha){pipeline_create_infos_t splats;R_CopyPipelineCreateInfos(&splats,&base);R_SetPipelineRenderPassVariant(&splats,SUBPASS_MAIN,variant);splats.vertex_input_state.vertexBindingDescriptionCount=0;splats.vertex_input_state.vertexAttributeDescriptionCount=0;splats.shader_stages[0].module=aftershock_alias_vert_module;splats.shader_stages[1].module=aftershock_alias_frag_module;splats.rasterization_state.cullMode=VK_CULL_MODE_NONE;R_CreateGraphicsPipeline(&vulkan_globals.aftershock_alias[variant][alpha],&splats,vulkan_globals.md5_pipelines[MAIN_RENDER_PASS_STANDARD][0].layout,"aftershock_alias");}
...
static void R_CreateShaderModules ()
{
	...
	CREATE_SHADER_MODULE (aftershock_structure_vert);
	CREATE_SHADER_MODULE (aftershock_vert);
    CREATE_SHADER_MODULE (holo_splat);
    CREATE_SHADER_MODULE (holo_structure);
    CREATE_SHADER_MODULE (holo_fragment);
    CREATE_SHADER_MODULE (holo_gore_vert);
    CREATE_SHADER_MODULE (holo_gore_frag);
	CREATE_SHADER_MODULE (aftershock_frag);
	CREATE_SHADER_MODULE (aftershock_fx_vert);
	CREATE_SHADER_MODULE (aftershock_alias_vert);
	CREATE_SHADER_MODULE (aftershock_alias_frag);
...
void R_CreatePipelines ()
{
	...
	DESTROY_SHADER_MODULE (aftershock_structure_vert);
	DESTROY_SHADER_MODULE (aftershock_vert);
    DESTROY_SHADER_MODULE (holo_splat);
    DESTROY_SHADER_MODULE (holo_structure);
    DESTROY_SHADER_MODULE (holo_fragment);
    DESTROY_SHADER_MODULE (holo_gore_vert);
    DESTROY_SHADER_MODULE (holo_gore_frag);
	DESTROY_SHADER_MODULE (aftershock_frag);
	DESTROY_SHADER_MODULE (aftershock_fx_vert);
	DESTROY_SHADER_MODULE (aftershock_alias_vert);
	DESTROY_SHADER_MODULE (aftershock_alias_frag);
	R_DestroyShaderModules ();
}
...
{
    for(int v=0;v<MAIN_RENDER_PASS_VARIANT_COUNT;++v) for(int f=0;f<2;++f) {
        vkDestroyPipeline(vulkan_globals.device,vulkan_globals.holo_splat[v][f].handle,NULL);
        vkDestroyPipeline(vulkan_globals.device,vulkan_globals.holo_structure[v][f].handle,NULL);
    }
    for(int v=0;v<MAIN_RENDER_PASS_VARIANT_COUNT;++v) vkDestroyPipeline(vulkan_globals.device,vulkan_globals.holo_gore[v].handle,NULL);
    HP_DestroyLayout();
	for(int variant=0;variant<MAIN_RENDER_PASS_VARIANT_COUNT;++variant){
		for(int full=0;full<2;++full)vkDestroyPipeline(vulkan_globals.device,vulkan_globals.aftershock_splat[variant][full].handle,NULL);
		for(int full=0;full<2;++full)vkDestroyPipeline(vulkan_globals.device,vulkan_globals.aftershock_structure[variant][full].handle,NULL);
		for(int alpha=0;alpha<2;++alpha)vkDestroyPipeline(vulkan_globals.device,vulkan_globals.aftershock_alias[variant][alpha].handle,NULL);
		vkDestroyPipeline(vulkan_globals.device,vulkan_globals.aftershock_solid[variant].handle,NULL);
		vkDestroyPipeline(vulkan_globals.device,vulkan_globals.aftershock_fx[variant].handle,NULL);
	}
	vkDestroyPipeline(vulkan_globals.device,vulkan_globals.aftershock_fx_oit.handle,NULL);vkDestroyPipeline(vulkan_globals.device,vulkan_globals.aftershock_fx_moment.handle,NULL);vkDestroyPipeline(vulkan_globals.device,vulkan_globals.aftershock_fx_composite.handle,NULL);
