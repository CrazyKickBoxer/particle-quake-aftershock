	...
	qboolean alphatest, vec3_t shadevector, vec3_t lightcolor, int showtris)
{
	...
	case PV_QUAKE3:
	{
        ...
        if(splats){vulkan_globals.vk_cmd_bind_descriptor_sets(cbx->cb,VK_PIPELINE_BIND_POINT_GRAPHICS,pipeline.layout.handle,3,1,&paliashdr->aftershock_set,0,NULL);VkBuffer ib;VkDeviceSize io;uint16_t *idx=(uint16_t *)R_IndexAllocate(12,&ib,&io);const uint16_t quad[6]={0,1,2,0,2,3};memcpy(idx,quad,12);vulkan_globals.vk_cmd_bind_index_buffer(cbx->cb,ib,io,VK_INDEX_TYPE_UINT16);vulkan_globals.vk_cmd_draw_indexed(cbx->cb,6,paliashdr->aftershock_samples * (((ubo->flags&0x130)==0x130)?2:1),0,0,0);break;}
