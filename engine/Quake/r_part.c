...
{
	if (color == 73 && count != 1024) { AS_GoreBlood (org, dir, q_min (count, 16));HP_Event(2,org,dir,8,130); }
	int			i, j;
	particle_t *p;
...
{
	if ((type & 127) == 2 || (type & 127) == 4) AS_GoreTrail (start, end);
	vec3_t		vec;
	float		len;
	int			j;
	particle_t *p;
	...
	while (len > 0)
	{
        ...
        if(AS_Neon() && (type==0 || type==1))p->die=q_min(p->die,cl.time+.16);
