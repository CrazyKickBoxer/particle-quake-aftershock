...
byte *SV_FatPVS (vec3_t org, qmodel_t *worldmodel) // johnfitz -- added worldmodel as a parameter
{
	...
	if(as_worldmode.value){memset(fatpvs,0xff,fatbytes);return fatpvs;}
	fatpvs_any = false;
...
void SV_SpawnServer (const char *server)
{
	...
	int as_seed_arg = COM_CheckParm("-test-seed");
	if (as_seed_arg && as_seed_arg+1<com_argc) COM_SeedRand((uint64_t)strtoull(com_argv[as_seed_arg+1],NULL,10));
