...
static int previous_holo_count,previous_native_count;
...
void R_AllocateLightmapComputeBuffers ()
{
	...
	previous_holo_count=previous_native_count=0;
	...
	buffer_create_info_t buffer_create_infos[4] = {
		{&lightstyles_scales_buffer, lightstyles_buffer_size, 0, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, (void **)&lightstyles_scales_buffer_mapped, NULL,
		 "Lightstyle scales"},
		{&lights_buffer, lights_buffer_size, 0, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, (void **)&lights_buffer_mapped, NULL, "Lights"},
...
void R_UpdateLightmapsAndIndirect (void *unused)
{
    ...
    const int native_dlights=num_used_dlights;

    if(previous_holo_count) {
        VkMemoryBarrier barrier={.sType=VK_STRUCTURE_TYPE_MEMORY_BARRIER,.srcAccessMask=VK_ACCESS_TRANSFER_WRITE_BIT|VK_ACCESS_UNIFORM_READ_BIT,.dstAccessMask=VK_ACCESS_TRANSFER_READ_BIT};
        vkCmdPipelineBarrier(cbx->cb,VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,VK_PIPELINE_STAGE_TRANSFER_BIT,0,1,&barrier,0,NULL,0,NULL);
        VkBufferCopy previous={(VkDeviceSize)(((current_compute_buffer_index^1)*MAX_DLIGHTS*2)+previous_native_count)*48,
            (VkDeviceSize)(current_compute_buffer_index*MAX_DLIGHTS*2+MAX_DLIGHTS+previous_native_count)*48,(VkDeviceSize)previous_holo_count*48};
        vkCmdCopyBuffer(cbx->cb,lights_buffer,lights_buffer,1,&previous);
        barrier.srcAccessMask=VK_ACCESS_TRANSFER_WRITE_BIT;barrier.dstAccessMask=VK_ACCESS_UNIFORM_READ_BIT;
        vkCmdPipelineBarrier(cbx->cb,VK_PIPELINE_STAGE_TRANSFER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,0,1,&barrier,0,NULL,0,NULL);
    }
    const int holo_count=r_dynamic.value?HP_CopyLights(cbx->cb,lights_buffer,(VkDeviceSize)(current_compute_buffer_index*MAX_DLIGHTS*2+native_dlights)*48,MAX_DLIGHTS-native_dlights):0;
    const qboolean holo_updates=holo_count>0 || previous_holo_count>0;
    num_used_dlights+=holo_count;
    previous_holo_count=holo_count;previous_native_count=native_dlights;
	...
	for (int lightmap_index = 0; lightmap_index < lightmap_count; ++lightmap_index)
	{
		...
		for (int y = 0; y < LMBLOCK_HEIGHT / LM_CULL_BLOCK_H; y++)
			for (int x = 0; x < LMBLOCK_WIDTH / LM_CULL_BLOCK_W; x++)
			{
				qboolean needs_update = holo_updates;
				for (int i = 0; i < native_dlights; i++)
				{
					float sq_dist = 0.0f;
					for (int j = 0; j < 3; j++)
					{
