...
void GLMesh_DeleteMeshBuffers (aliashdr_t *mainhdr)
{
	...
	for (aliashdr_t *hdr = mainhdr; hdr != NULL; hdr = hdr->nextsurface)
	{
		...
		if (in_update_screen)
		{
			AddBufferGarbage (hdr->vertex_buffer, VK_NULL_HANDLE, hdr->vertex_allocation, hdr->aftershock_set, hdr->aftershock_set ? &vulkan_globals.joints_buffer_set_layout : NULL);
		...
		else
		{
			...
			if(hdr->aftershock_set)R_FreeDescriptorSet(hdr->aftershock_set,&vulkan_globals.joints_buffer_set_layout);
		...
		hdr->aftershock_set=VK_NULL_HANDLE;
	...
	int num_skeleton_indexes)
{
	...
	hdr->aftershock_offset=totalvbosize/4;hdr->aftershock_samples=as_count;totalvbosize+=as_count*16;
	// create the vertex buffer (empty)
	TEMP_ALLOC (byte, vbodata, totalvbosize);
	if(as_count)memcpy(vbodata+hdr->aftershock_offset*4,as_samples,as_count*16);Mem_Free(as_samples);
	...
	// Allocate vertex buffer & upload to GPU
	{
		...
		if(as_count){hdr->aftershock_set=R_AllocateDescriptorSet(&vulkan_globals.joints_buffer_set_layout);VkDescriptorBufferInfo buffer={hdr->vertex_buffer,0,totalvbosize};VkWriteDescriptorSet write={VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};write.dstSet=hdr->aftershock_set;write.dstBinding=0;write.descriptorCount=1;write.descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;write.pBufferInfo=&buffer;vkUpdateDescriptorSets(vulkan_globals.device,1,&write,0,NULL);}
