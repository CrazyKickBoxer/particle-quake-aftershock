...
static FILE *showcase_pcm;
static void S_CloseShowcasePCM(void) { if(showcase_pcm){fclose(showcase_pcm);showcase_pcm=NULL;} }
static void S_TransferPaintBuffer (int endtime)
{
    int tick=AS_ShowcaseTick(),arg=COM_CheckParm("-capture-audio");
    if(arg && arg+1<com_argc && (COM_CheckParm("-capture-demo") ? tick>=0 : (tick>=31 && tick<=330))) {
        if(!showcase_pcm) {
            showcase_pcm=Sys_fopen(com_argv[arg+1],"wb");
            if(!showcase_pcm)Sys_Error("Cannot open showcase audio output");
            atexit(S_CloseShowcasePCM);
            Con_Printf("Showcase audio: %d Hz stereo PCM16\n",shm->speed);
        }
        short samples[PAINTBUFFER_SIZE*2];
        int count=endtime-paintedtime;
        for(int i=0;i<count;++i){samples[i*2]=CLAMP(-32768,paintbuffer[i].left/256,32767);samples[i*2+1]=CLAMP(-32768,paintbuffer[i].right/256,32767);}
        if(fwrite(samples,4,count,showcase_pcm)!=(size_t)count)Sys_Error("Showcase audio write failed");
        fflush(showcase_pcm);
    }
