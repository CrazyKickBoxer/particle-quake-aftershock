...
===============
*/
#include "aftershock_fidelity.inc"
...
static void GL_CreateColorBuffer (void)
{
...

	{
		...
		vkGetPhysicalDeviceImageFormatProperties (
			vulkan_physical_device, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, image_create_info.usage, 0,
...
void GL_BeginRenderingTask (void *unused)
{
    ...
    HP_Run(vulkan_globals.primary_cb_contexts[0].cb,current_cb_index);
}
...
void WriteScreenshot (VkBuffer buffer, vulkan_memory_t memory)
{
	...
	if (!COM_CheckParm("-screenshot") && !COM_CheckParm("-capture-sequence") && Steam_SaveScreenshot (buffer_ptr, glwidth, glheight))
...
task_handle_t GL_EndRendering (qboolean use_tasks, qboolean swapchain)
{
	end_rendering_parms_t parms = {
        ...
        .neon = AS_Neon(),
        .fidelity = AS_Neon() && as_fidelity.value != 0 && vid_palettize.value == 0,
        .reflections = as_reflections.value != 0,
...
{
	// The previous render owns and clears the screenshot request flag.
	GL_SynchronizeEndRenderingTask ();
	...
	if(Cmd_Argc()>=4)q_strlcpy(screenshot_imagename,Cmd_Argv(3),sizeof(screenshot_imagename));
