...
void Sys_Init (void)
{
	...
	{
		static char aftershock_userdir[MAX_OSPATH];
		int arg=COM_CheckParm("-userdir");
		if(arg && arg+1<com_argc){q_strlcpy(aftershock_userdir,com_argv[arg+1],sizeof(aftershock_userdir));Sys_mkdir(aftershock_userdir);host_parms->userdir=aftershock_userdir;}
	}
