...
static void M_Menu_Holo_f(void);
...
static void M_GraphicsOptions_AdjustSliders (int dir, qboolean mouse)
{
	...
	if (!vulkan_globals.ray_query && option >= GRAPHICS_OPT_SHADOWS)
		++option;
    if(!vulkan_globals.screen_effects_sops && option>=GRAPHICS_OPT_AMBIENT_OCCLUSION) ++option;
	switch (option)
	{
    case GRAPHICS_OPT_HOLO: if(dir) M_Menu_Holo_f();break;
...
static void M_GraphicsOptions_Draw (cb_context_t *cbx)
{
    ...
    M_Print(cbx,MENU_LABEL_X,top+CHARACTER_SIZE*(M_GraphicsOptions_NumItems()-1),"Reactive Physics...");
	// cursor
...
//=============================================================================
/* Holo controls edit only renderer cvars. */
static const struct { const char *label,*cvar;float min,max,step; } holo_options[]={
    {"Enabled","r_holo_physics",0,1,1},{"Blast Strength","r_holo_phys_blast",0,4,.1f},
    {"Suction","r_holo_phys_suction",0,1,.05f},{"Impact Sprays","r_holo_phys_spray",0,4,.1f},
    {"Gore","r_holo_phys_gore",0,1,1},{"Gore Lifetime","r_holo_phys_gore_life",1,90,5},
    {"Death Dissolve","r_holo_phys_death",0,4,.1f},{"Projectile Wake","r_holo_phys_wake",0,2,.1f},
    {"Dust","r_holo_phys_dust",0,2,.1f},{"Turbulence","r_holo_phys_turbulence",0,2,.1f},
    {"Debris Lights","r_holo_phys_debris_light",0,1,1},{"Particle Budget","r_holo_phys_budget",1024,262144,16384}
};
static int holo_cursor;
static void M_Menu_Holo_f(void) { IN_Deactivate(true);key_dest=key_menu;m_state=m_holo;m_entersound=true; }
static void M_Holo_Key(int key) {
    if(key==K_ESCAPE || key==K_MOUSE2 || key==K_BBUTTON) { M_Menu_GraphicsOptions_f();return; }
    if(key==K_UPARROW || key==K_DOWNARROW) { holo_cursor=(holo_cursor+(key==K_UPARROW?countof(holo_options)-1:1))%countof(holo_options);S_LocalSound("misc/menu1.wav");return; }
    if(key==K_LEFTARROW || key==K_RIGHTARROW || key==K_ENTER || key==K_KP_ENTER || key==K_MOUSE1 || key==K_ABUTTON) {
        int i=holo_cursor;float value=Cvar_VariableValue(holo_options[i].cvar);
        value=(holo_options[i].max==1 && holo_options[i].step==1)?!value:CLAMP(holo_options[i].min,value+(key==K_LEFTARROW?-1:1)*holo_options[i].step,holo_options[i].max);
        Cvar_SetValue(holo_options[i].cvar,value);
        if(i==0 && value) Cvar_SetValue("r_holo",1);
        S_LocalSound("misc/menu3.wav");
    }
}
static void M_Holo_Draw(cb_context_t *cbx) {
    M_PrintWhite(cbx,88,24,"REACTIVE PHYSICS");
    for(int i=0;i<countof(holo_options);++i) {
        int y=44+i*10;float value=Cvar_VariableValue(holo_options[i].cvar);
        M_Print(cbx,24,y,holo_options[i].label);
        if(holo_options[i].max==1 && holo_options[i].step==1) M_DrawCheckbox(cbx,180,y,value!=0);
        else M_DrawSlider(cbx,180,y,(value-holo_options[i].min)/(holo_options[i].max-holo_options[i].min),holo_options[i].max>4?va("%.0f",value):va("%.2g",value));
    }
    M_Mouse_UpdateListCursor(&holo_cursor,8,320,44,10,countof(holo_options),0);
    Draw_Character(cbx,8,44+holo_cursor*10,12+((int)(realtime*4)&1));
    M_PrintWhite(cbx,24,178,"Holo Physics - renderer only");
}
...
void M_Init (void)
{
    ...
    Cmd_AddCommand("menu_holo_physics",M_Menu_Holo_f);
